...
function createWindowState(
  ...
  const width = embedded ? Math.min(definition.width, 1060) : definition.width
  const height = embedded ? Math.min(definition.height, 604) : definition.height
  return {
    appId,
    title: definition.title,
    icon: <definition.icon size={15} />,
    x: (embedded ? 78 : 70) + offset * (embedded ? 28 : 34),
    y: (embedded ? 42 : 52) + offset * (embedded ? 24 : 30),
    width,
    height,
    zIndex,
    minimized: false,
...
export function VirtualDesktop({ initialApp, embedded = false }: { initialApp: ExhibitionAppId | null; embedded?: boolean }) {
  ...
  const openApp = useCallback((appId: ExhibitionAppId) => {
    setStartOpen(false)
    setWindows((current) => {
      ...
      zCounter.current += 1
      return [...current, createWindowState(appId, current.length % 6, zCounter.current, embedded)]
    })
  }, [embedded])
  ...
  return (
    <div
      className={`virtual-os ${embedded ? 'virtual-os--embedded' : ''}`}
      onMouseDown={(event) => {
    ...
    >
      ...
      <div className="virtual-os__info-widget">
        ...
        <p>点击桌面图标启动应用，或使用底部任务栏。展厅导航与设备控制会实时联动三维场景。</p>
